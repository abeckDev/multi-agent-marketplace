...
async def run_experiment_background(experiment_config: ExperimentCreate, experiment_name: str):
...
async def list_datasets():
    ...
    for item in data_dir.iterdir():
        ...
        if businesses_dir.exists() and customers_dir.exists():
            ...
            num_businesses = len(list(businesses_dir.glob("*.yaml"))) + len(list(businesses_dir.glob("*.yml")))
            num_customers = len(list(customers_dir.glob("*.yaml"))) + len(list(customers_dir.glob("*.yml")))
...
async def create_experiment(
    ...
    if not dataset_path.exists():
        raise HTTPException(
            status_code=400,
            detail=f"Dataset not found: {experiment_config.dataset}"
        )
    ...
    if not businesses_dir.exists() or not customers_dir.exists():
        raise HTTPException(
            status_code=400,
            detail=f"Invalid dataset: missing businesses/ or customers/ directories"
        )
    ...
    background_tasks.add_task(run_experiment_background, experiment_config, experiment_name)
...
async def get_experiment_status(name: str):
    ...
    if name not in experiment_tracker:
        raise HTTPException(
            status_code=404,
            detail=f"Experiment '{name}' not found in tracker"
        )
...
async def list_experiments(
    ...
    try:
        ...
        raise HTTPException(
            status_code=401,
            detail="Invalid database password"
        )
    except Exception as e:
